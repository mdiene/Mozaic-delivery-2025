...
export const Fleet = () => {
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      ...
      alert(`Échec: ${JSON.stringify(error)}`);
    }
  };

  const handleTruckSelect = (truck: TruckType) => {
    setFormData({ ...formData, ...truck, id: truck.id });
                  ...
                  <tr key={truck.id}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <button onClick={() => handleEdit(truck)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm disabled:opacity-30"><Edit2 size={16} /></button>
                        <button onClick={() => handleDelete(truck.id)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm btn-text-error disabled:opacity-30"><Trash2 size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {[...assignedDrivers, ...unassignedDrivers].map(driver => (
                  <tr key={driver.id}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => handleAssignTruck(driver)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm text-emerald-600 disabled:opacity-30"><LinkIcon size={16} /></button>
                      <button onClick={() => handleEdit(driver)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm disabled:opacity-30"><Edit2 size={16} /></button>
                      <button onClick={() => handleDelete(driver.id)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm btn-text-error disabled:opacity-30"><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              {activeTab === 'trucks' ? (
                <>
                  ...
                  <input required className="w-full border border-input rounded-lg p-2 text-sm uppercase" value={truckSearch} onChange={(e) => { setTruckSearch(e.target.value.toUpperCase()); setFormData({ ...formData, plate_number: e.target.value.toUpperCase() }); }} />
                  <label className="block text-sm font-medium">Capacité (T)</label>
                  <input type="number" required className="w-full border border-input rounded-lg p-2 text-sm" value={formData.capacity_tonnes || ''} onChange={e => setFormData({...formData, capacity_tonnes: e.target.value})} />
                </>
              ) : (
                <>
                  <label className="block text-sm font-medium">Nom Complet</label>
                  <input required className="w-full border border-input rounded-lg p-2 text-sm" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} />
                  <label className="block text-sm font-medium">Téléphone</label>
                  <input required className="w-full border border-input rounded-lg p-2 text-sm" value={formData.phone || ''} onChange={e => setFormData({...formData, phone: e.target.value})} />
                </>
              )}
          ...
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-sm overflow-hidden border border-border">
            ...
            <div className="p-6 flex flex-col items-center gap-4">
               ...
               <div className="w-full space-y-2">
                  {!generatedQRValue && <button onClick={handleGenerateQR} disabled={isVisitor} className="w-full bg-blue-600 text-white py-2 rounded-lg disabled:opacity-30">Générer QR</button>}
                  {generatedQRValue && <><button onClick={handleSaveQR} disabled={isVisitor} className="w-full bg-emerald-600 text-white py-2 rounded-lg disabled:opacity-30">Enregistrer</button><button onClick={handlePrintQR} className="w-full bg-card border py-2 rounded-lg">Imprimer</button></>}
