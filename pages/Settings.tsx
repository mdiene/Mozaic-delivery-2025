...
export const Settings = () => {
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    } catch (error) { alert('Échec de l\'enregistrement.'); }
  };
      ...
      {activeTab === 'operators' && (
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => openModal('operator')} disabled={isVisitor} className="flex items-center gap-2 bg-primary text-primary-foreground px-3 py-1.5 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed">Ajouter Opérateur</button>
          </div>
              ...
              {operators.filter(op => selectedPhaseFilter === 'all' ? true : op.projet_id === selectedPhaseFilter).map(op => (
                <tr key={op.id}>
                  ...
                  <td className="text-right">
                    <button onClick={() => handleEdit('operator', op)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed"><Edit2 size={18} /></button>
                    <button onClick={() => handleDelete('operators', op.id)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm btn-text-error disabled:opacity-30 disabled:cursor-not-allowed"><Trash2 size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {activeTab === 'projects' && (
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
          <div className="flex justify-between mb-6">
            ...
            <button onClick={() => openModal('project')} disabled={isVisitor} className="bg-primary text-white px-3 py-1.5 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed">Ajouter Projet</button>
          </div>
          <table className="table">
            ...
            <tbody>
              {projects.map(p => (
                <tr key={p.id}>
                  ...
                  <td className="text-right">
                    <button onClick={() => handleEdit('project', p)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed"><Edit2 size={18} /></button>
                    <button onClick={() => handleDelete('project', p.id)} disabled={isVisitor || usedProjectIds.has(p.id)} className="btn btn-circle btn-text btn-sm btn-text-error disabled:opacity-30 disabled:cursor-not-allowed"><Trash2 size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {activeTab === 'geographic' && (
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
          <div className="flex justify-between mb-6">
            ...
            <button onClick={() => openModal(geoTab.slice(0, -1))} disabled={isVisitor} className="bg-primary text-white px-3 py-1.5 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed">Ajouter {geoTab.slice(0,-1)}</button>
          </div>
          <table className="table">
            <thead><tr><th>Nom</th><th>Code</th><th className="text-right">Actions</th></tr></thead>
            <tbody>
              {geoTab === 'regions' && regions.map(item => (
                <tr key={item.id}><td>{item.name}</td><td>{item.code}</td><td className="text-right"><button onClick={() => handleEdit('region', item)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed"><Edit2 size={16} /></button><button onClick={() => handleDelete('regions', item.id)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm btn-text-error disabled:opacity-30 disabled:cursor-not-allowed"><Trash2 size={16} /></button></td></tr>
              ))}
              {geoTab === 'departments' && departments.map(item => (
                <tr key={item.id}><td>{item.name}</td><td>{item.code}</td><td className="text-right"><button onClick={() => handleEdit('department', item)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed"><Edit2 size={16} /></button><button onClick={() => handleDelete('departments', item.id)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm btn-text-error disabled:opacity-30 disabled:cursor-not-allowed"><Trash2 size={16} /></button></td></tr>
              ))}
              {geoTab === 'communes' && communes.map(item => (
                <tr key={item.id}><td>{item.name}</td><td>{item.code}</td><td className="text-right"><button onClick={() => handleEdit('commune', item)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed"><Edit2 size={16} /></button><button onClick={() => handleDelete('communes', item.id)} disabled={isVisitor} className="btn btn-circle btn-text btn-sm btn-text-error disabled:opacity-30 disabled:cursor-not-allowed"><Trash2 size={16} /></button></td></tr>
              ))}
            </tbody>
          </table>
          <div className="flex gap-4 mt-6 border-t pt-4">
             <button onClick={() => setGeoTab('regions')} className={`text-xs font-bold uppercase tracking-wider ${geoTab === 'regions' ? 'text-primary' : 'text-muted-foreground'}`}>Régions</button>
             <button onClick={() => setGeoTab('departments')} className={`text-xs font-bold uppercase tracking-wider ${geoTab === 'departments' ? 'text-primary' : 'text-muted-foreground'}`}>Départements</button>
             <button onClick={() => setGeoTab('communes')} className={`text-xs font-bold uppercase tracking-wider ${geoTab === 'communes' ? 'text-primary' : 'text-muted-foreground'}`}>Communes</button>
          </div>
        </div>
      )}
    </div>
