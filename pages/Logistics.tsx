    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          disabled={isVisitor}
          className="flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-lg font-medium transition-colors shadow-sm disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed"
        >
          <Plus size={18} />
          Nouvelle Expédition
        </button>
      ...
      <div className="accordion flex flex-col gap-4 min-h-[500px]">
        {groupedDeliveries.length === 0 && (
          <div className="p-12 text-center text-muted-foreground bg-card rounded-xl border border-border">
             {searchTerm || mainPhaseFilter !== 'all' ? 'Aucun résultat.' : 'Aucune expédition.'}
          </div>
        )}
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 bg-card">
                <div className="space-y-4">
                  ...
                  <div className="overflow-x-auto pb-2 -mx-1 px-1">
                     <form className="filter">
                        <input className="btn btn-square" type="reset" value="×" onClick={() => setModalPhaseFilter('all')} />
                        <input className="btn" type="radio" name="modal-phase" aria-label="Toutes" checked={modalPhaseFilter === 'all'} onChange={() => setModalPhaseFilter('all')} />
                        {projects.map(p => (<input key={p.id} className="btn" type="radio" name="modal-phase" aria-label={`Phase ${p.numero_phase}`} checked={modalPhaseFilter === p.id} onChange={() => setModalPhaseFilter(p.id)} />))}
                     </form>
                  </div>
                  ...
                  {selectedAllocation && (<div className="mt-3 grid grid-cols-3 gap-2 text-center bg-muted/30 p-3 rounded-lg border border-border border-dashed"><div className="flex flex-col"><span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Cible</span><span className="font-mono text-sm font-medium text-foreground">{targetTonnage} T</span></div><div className="flex flex-col border-l border-border"><span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Livré</span><span className="font-mono text-sm font-medium text-primary">{calculatedDelivered} T</span></div><div className="flex flex-col border-l border-border"><span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Restant</span><span className="font-mono text-sm font-medium text-foreground">{remainingTonnage} T</span></div></div>)}
                  <div className="p-4 bg-muted/50 rounded-lg border border-border"><div className="flex justify-between items-center mb-2"><span className="text-sm font-medium text-foreground">Numéro BL</span><button type="button" onClick={() => setFormData({...formData, bl_number: generateBL()})} className="text-xs flex items-center gap-1 text-primary hover:underline"><RefreshCw size={12} /> Régénérer</button></div><input type="text" required className="w-full border border-input rounded-lg p-2 text-lg font-mono font-bold bg-background text-foreground tracking-wide" value={formData.bl_number || ''} onChange={(e) => setFormData({...formData, bl_number: e.target.value.toUpperCase()})} /></div>
                </div>
                <div className="space-y-4">
                   ...
                   <select className="w-full border border-input rounded-lg p-2 text-sm bg-background text-foreground" value={formData.driver_id || ''} onChange={(e) => setFormData({...formData, driver_id: e.target.value})}><option value="">Sélectionner Chauffeur...</option>{drivers.filter(d => !d.truck_id || (formData.truck_id && d.truck_id === formData.truck_id) || d.id === formData.driver_id).map(d => <option key={d.id} value={d.id}>{d.name} {d.truck_id ? '(Assigné)' : ''}</option>)}</select>
